...
export class Solitaire {
  ...
  public newGame(): void {
     ...
     *
     * Custom distribution:
     * - Piles 0 and 6: 2 cards each
     * - Piles 1 and 5: 5 cards each
     * - Piles 2 and 4: 7 cards each
     * - Pile 3: 7 cards
     *
    ...
    // Custom distribution of cards
    const pileCardCounts = [2, 5, 7, 7, 7, 5, 2];
    
    // Deal cards to each pile according to the specified counts
    for (let i = 0; i < 7; i++) {
      for (let j = 0; j < pileCardCounts[i]; j++) {
        const card = this.#deck.draw() as Card;
        // Only the top card of each pile is face up
        if (j === pileCardCounts[i] - 1) {
          card.flip();
        }
